...
\begin{wrapfigure}{r}{0.3\textwidth}
	...
	\resizebox{0.3\textwidth}{!}{
		...
		\draw[edge] (q1)  -- (q3)  node[midway, above, sloped] {\swFmt{9d}/\swFmt{10v}};
		\draw[edge] (q2)  -- (q3)  node[pos=0.3, above, sloped] {\swFmt{9v}/\swFmt{10v}};
		\draw[edge] (q4)  -- (q3)  node[midway, above, sloped] {\swFmt{10d}};
		%\draw[edge] (q4) to node[pos=.1] {f1} to node[midway] {10d} to node[pos=.9] {f2} (q3);
		\draw[edge] (q3) -- (q8)  node[midway, above, sloped] {\swFmt{11v}};
		\draw[edge] (q3) -- (q7) node[midway, above, sloped] {\swFmt{11d}};
		\draw[edge] (q7)  -- (q5)  node[midway, above, sloped] {\swFmt{12d}};
		\draw[edge] (q7)  -- (q6) node[midway, above, sloped] {\swFmt{12v}};
		\draw[edge] (q5) -- (q4) node[midway, above, sloped] {\swFmt{13d}};
		\draw[edge] (q6) -- (q4) node[midway, above, sloped] {\swFmt{13v}};
		\end{tikzpicture}
	}
...
\paragraph{} 
La modélisation est un outil essentiel pour décrire de manière formelle un système\todoMV{Le modèle est un outil, la modélisation est une étape, dont le but est d'avoir un double formel du système réel.} afin d'en extraire un comportement se rapprochant de la réalité et d'observer son évolution dans le temps. De cette manière, il est possible d'étudier ce système pour trouver des propriétés ou résoudres des problèmes.
...
Nous allons donc commencer par modéliser la maquette \todo{pas ok avec le pourquoi, à revoir} pour obtenir un espace dans lequel nos trains évolueront. Cependant, elle\todoMV{C'est qui "elle" ?} contient beaucoup de données inutiles, on va donc se concentrer sur les éléments qui nous intéressent en la représentant sous forme de graphe, c'est la figure \ref{fig:grapheMaquette}.
...
[parler de \suc{b_i}{d}{\sigma} qui traduit le graphe en fonction]\todoMV{}
...
\paragraph{} 
Dans cette section, nous allons aborder différents modèles et leurs différences\todoMV{Pourquoi ? Parce que c'est plus simple/pour la clarté de la présentation/pour expliquer le cheminement qui a mené au modèle final... }, il est important de comprendre notre méthode de travail pour mieux appréhender les évolutions futures.
...
\paragraph{aiguillages.}\todoMV{Capitale}
La modélisation des aiguillages est constante sur les différents modèles, nous allons donc la définir\todoMV{présenter} ici. Nous ne travaillons qu'avec des aiguillages "1-to-2", c'est à dire qu'il permet de choisir entre deux voies.
...
\end{itemize}
Vous l'avez sans doute remarqué mais\todoMV{Formulation (e.g.\ "À noter, ...")} les aiguillages possèdent des identifiants dans la continuité des cantons, ils sont considérer comme des cantons particuliers. Dans notre cas, les identifiants des cantons vont de 1 à 8 et ceux des aiguillages de 9 à 13.
...
\paragraph{} Notre premier modèle sera\todoMV{"est". Règle générale : écrit tout au présent, ça évite de se prendre la tête.} simple, il va permettre\todoMV{il permet} d'entrer dans le sujet, de se familiariser avec les concepts nouveaux, notamment les règles de transitions, et de servir de base pour les futures évolutions.
...
Une ressources devient critique lorsqu'elle est convoitée par plusieurs trains, dans notre cas, il s'agit des cantons.\todoMV{TB}
Parfois, il y a une manière intelligente de considérer les ressources critiques~:

\begin{example}[Ressources critiques]
	~\\
	Soient $c_1 = [1,2,3]$ et $c_2 = [4,2,1]$ les cantons empruntés par les trains $t_1$ et $t_2$\todoMV[inline]{"... par deux trains $t_1$ et $t_2$."}.
...
\\\todoMV{Évite les retours à la ligne manuels comme ça. Il vaut mieux sauter une ligne pour que LateX créé un nouveau paragraphe avec indentation etc.}Une seconde\todoMV{Si tu présente deux problématiques, il faut les anticiper dans l'intro de la section; d'autnat que le paragraphe commence par "Au travers de ce modèle, nous cherchons à résoudre un des problèmes". À la place, tu peux commencer par : "Ce premier modèle nous permet de résoudre deux problèmes : la gestion des sections critique et l'actionnement des aiguillages." puis continuer.} problèmatique\todoMV{orth} de taille à émergée pendant la réalisation de ce modèle~: le déclenchement des évènements doit se faire avant un moment précis.
Par exemple, si un train à\todoMV{ortho} besoin qu'un aiguillage soit dévié pour continuer sa route, il faut être certain que le régulateur à\todoMV{ortho} exécuté l'évènement de déviation avant que le train n'arrive à l'aiguillage.
...
Nous verrons comment résoudre cela plus loin.\todoMV{La formulation de ces dernières phrases est un peu maladroite. Tu peux aller à l'essentiel : "Il faut nous assurer que l'aiguillage est dans la bonne position avant que le train ne le franchisse, si possible sans arrêter le train."}
...
\begin{sidebox}
	...
	\end{grammar}
	\todoMV[inline]{Pas besoin d'avoir une règle pour les entiers, d'autant que ceux sont des règles syntaxiques.}
	\vspace{0.3cm}\\
\end{sidebox}
...
\noindent
Les informations que le train possède sont limitées, nous l'avons construit dans l'optique d'un "train ignorant" dont la fonction est de se déplacer en fonction de son programme interne\todoMV{Peut-être qu'il faut préciser cette approche générale au début.}.
...
\begin{itemize}
	\item Les jetons : Notés \textbf{J}, ils représentent l'accès à une ressource, ici un canton. Il suffit d'incrémenter la valeur du jeton pour libérer cette ressource.
	 On les définit par $J~:~\posFmt{\mathcal{B}} \mapsto \nats$\todoMV{Le $J$ n'est pas le même que juste avant (pareil pour les autres éléments)}.
	\item La file d'attente : Noté\todoMV{ortho} \textbf{W}, elle permet de garder en mémoire les trains qui attendent une ressource, nous verrons plus loin sont utilité. Elle est définie par $W~:~\posFmt{\mathcal{B}} \times \nats \mapsto \trainFmt{\gamma}$.
...
\paragraph{Évènements.}\todoMV{N'hésite pas à utiliser des sous-paragraphes pour donner les différentes étapes de l'explication.}
Comme vue\todoMV{ortho} avant, chaque train est rattaché, virtuellement par le régulateur, à un gestionnaire d'évènements pour traiter les évènements qui le concernent.\todoMV{Formulation bizarre}
...
Pour ne pas nous rajouter ce problème en plus, nous avons opter pour la forme actuelle~: $g_1 = [[-],[-],\dots]$. Ainsi, l'index de la liste d'ordre agit comme un numéro d'évènement.\todoMV{Ce qui précède n'est pas hyper clair (mais c'est dur à expliquer). Peut-être qu'il vaut mieux commencer par donner la forme de $g$ avec un exemple, et d'expliquer les alternatives envisagées dans une boite annexe.}
Ce qui nous amène au cas des évènements vides. Il est nécessaire que chaque déplacement de train entraîne un évènement, même si celui-ci est vide, l'index de la liste d'ordre servant de numéro d'évènement celle-ci s'en retrouverait décalé dans le cas contraire.
...
Or, la suppréssion\todoMV{ortho} des évènements vides n'est pas une assez grande motivation pour justifier la création d'une règles supplémentaire.
...
\noindent
La règle \ruleDef{Until} permet de déplacer un train $\trainFmt{t_m} \in \trainFmt{\Gamma}$ sur un canton adjacent.\todoMV{Si c'est ce qui est important, commence par expliquer ça : comment je vois, dans la règle, que le train à changé de canton ? Dis que $b_i$ devient $b_j$ (cohérent avec suc), et ensuite, dans un second temps, explique dans quels cas on peut appliquer cette règle.} Si on regarde du côté des prémisses, on retrouve notre demande d'autorisation~: $A[\trainFmt{t_m}] \neq 0$, le train est immobilisé si le compteur tombe à 0.
Pour trouver le prochain canton, on utilise la fonction $\suc{\posFmt{b_i}}{\dirFmt{d}}{\swFmt{\sigma}}$ définit\todoMV{ortho} plus tôt. Enfin, la dernière prémisse est un peu particulière~: ${\tt len}(E[\trainFmt{t_m}][{\text Ne}[\trainFmt{t_m}]]) = 0$, elle nous assure que le gestionnaire d'évènements du train est vide.
Cela règle le second point de la problèmatique\todoMV{ortho} de ce modèle. (pas convaincu que les gens comprennent pour le dernier)
...
\noindent
Les règles doivent pouvoir traiter les ordres des évènements, on pourrait s'attendre à avoir une règle \ruleDef{Wait} et une règle \ruleDef{Incr}\todoMV{Qui correspondraient à quoi ? Explique d'abord l'objectif général, puis l'approche intuitive avec une paire de règle, puis pourquoi on a en fait 2 paires}. Cependant, nous avons besoin de comportements plus fin, nous allons voir ça dès maintenant. 
...
Si l'on regarde les prémisses de la règle d'incrémentation, on remarque que l'on fait appel à la fonction $nextWait(\trainFmt{w_{id}},E[\trainFmt{w_{id}}])$ qui cherche dans le gestionnaire d'évènements du train \trainFmt{$t_m$} le prochain ordre $wait(j,x)$ et calcule le nombre de cantons avant d'y accéder, s'il n'existe pas, il retourne la distance à l'arrivée.
...
\paragraph{Limitations.}\todoMV{TB}
...
\begin{wrapfigure}{r}{0.3\textwidth}
	...
	\resizebox{0.3\textwidth}{!}{
		...
		\node[vertex] (q3)	  {3};
		\node[vertex] (q4) [right=6cm of q3] {4};
		\node[vertex] (q2) [above right=1cm and 4cm of q3] {2};
		\node[vertex] (q1) [above=of q2] {1};
		\node[vertex] (q5) [below=3cm of q4] {5};
		\node[vertex] (q7) [below=3cm of q3] {7};
		\node[vertex] (q8) [below right=1cm and 1.5cm of q3] {8};
		\node[vertex] (q6) [below left=1.5cm and 2cm of q4] {6};

		\draw[edge] (q1)  -- (q3)  node[midway, above, sloped] {\swFmt{9d}/\swFmt{10v}} node[pos=0.1,above,sloped] {1L} node[pos=0.9,above,sloped] {3R};
		\draw[edge] (q2)  -- (q3)  node[pos=0.4, above, sloped] {\swFmt{9v}/\swFmt{10v}} node[pos=0.1,above,sloped] {2L} node[pos=0.9,above,sloped] {3R};
		\draw[edge] (q4)  -- (q3)  node[midway, above, sloped] {\swFmt{10d}} node[pos=0.1,above,sloped] {4L} node[pos=0.9,above,sloped] {3R};
		\draw[edge] (q3) -- (q8)  node[midway, above, sloped] {\swFmt{11v}} node[pos=0.1,above,sloped] {3L} node[pos=0.9,above,sloped] {8R};
		\draw[edge] (q3) -- (q7) node[midway, above, sloped] {\swFmt{11d}} node[pos=0.1,above,sloped] {3L} node[pos=0.9,above,sloped] {7R};
		\draw[edge] (q7)  -- (q5)  node[midway, above, sloped] {\swFmt{12d}} node[pos=0.1,above,sloped] {7L} node[pos=0.9,above,sloped] {5R};
		\draw[edge] (q7)  -- (q6) node[midway, above, sloped] {\swFmt{12v}} node[pos=0.1,above,sloped] {7L} node[pos=0.9,above,sloped] {6R};
		\draw[edge] (q5) -- (q4) node[midway, above, sloped] {\swFmt{13d}} node[pos=0.1,above,sloped] {5L} node[pos=0.9,above,sloped] {4R};
		\draw[edge] (q6) -- (q4) node[midway, above, sloped] {\swFmt{13v}} node[pos=0.1,above,sloped] {6L} node[pos=0.9,above,sloped] {4R};
		\end{tikzpicture}
	}
	\caption{Graphe modélisant le réseau de la maquette avec les feux de signalisation}
	\label{fig:grapheMaquetteM1}
\end{wrapfigure}

Le modèle 0 terminé, il est l'heure de recommencer l'itération pour donner vie au modèle 1.
\todo{Figure \ref{fig:grapheMaquetteM1} surchargée ??}
...
\begin{sidebox}
	...
	\begin{grammar}
		...
		\firstcasesubtil{section}{ 1..\infty }{}
	\end{grammar}
\end{sidebox}
...
\todo{La figure \ref{fig:infos} est compréhensible ?}


\paragraph{Évènements.} \todo[inline]{Ajout de Auth}

\begin{example}[GoodEnding M1]
	~\\
	Modélisation de l'état initale d'un scénario en utilisant le modèle 1~: (goodEnding / mettre schéma en annexe ?)
	\\$B~: []$ \commentaire{// vide}
	\vspace{0.3cm}
	\\$\Gamma~: \{t_1, t_2\}$ avec,
	\begin{itemize}
	...
	\begin{itemize}
		...
		\item[] $G = \texttt{false}$
		\item[] $H = [0, 0]$
		\item[] $F = [(\posFmt{3},\dirFmt{f}) : \sigred,(\posFmt{4},\dirFmt{b}) : \sigred]$ \commentaire{// De base, tous les feux sont au vert}
	\end{itemize}
	\vspace{0.3cm}
	$g_1 = [[],[],[\turnOrder{1}{v}, \incr{2}]]$
	\\$g_2 = [[\wait{2}{1}],[],[]]$
...
%comportement fail safe des feux
%ajout du numéro de canton dans le gestionnaire, mais pas de problème de cohérence :  c'est pas la position actuel du train mais l'endroit où le train à déclencher l'event, comme histo du modèle 0
