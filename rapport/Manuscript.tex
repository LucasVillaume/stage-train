...
\begin{titlepage}
    ...
    {\Huge \textbf{Vérification formelle de programmes d'ordonnancement de trains}}\\[3cm]
    
    {\huge\textbf{Lucas Villaume}}\\[0.5cm]
    {\Large Encadré par : \textbf{Guillaume Bonfante} \& \textbf{Martin Vassor}}\\[2cm]
    


    {\Large Master SIRAV, parcours SIS}\\[2cm]
    
    {\Large \today}\\[4cm]
    

    \includegraphics[width=0.3\textwidth]{ul.png}\hspace{4cm}
    \includegraphics[width=0.2\textwidth]{loria.png}\par\vspace{1cm}
...
Je souhaite remercier toutes les personnes qui ont gravité autour de ce stage, en particulier mes encadrants, très présents et bienveillants.
Merci également aux membres de l'équipe Véridis pour l'accueil chaleureux et leur soutien tout au long de cette expérience.
...
C'est dans ce contexte que s'inscrit la création d'une maquette de train, simulant un réseau ferroviaire à l'échelle d'une ville. L'objectif de cette dernière est double, elle est d'abord utilisée pour offrir une plateforme d'entraînement aux étudiants participant à Cyber Humanun Est (CHE) \cite{CHE}, dans l'optique de leur faire découvrir la défense et l'attaque de ce type de système.
Le second objectif vise à reproduire des déplacements ferroviaires réalistes afin d'identifier les problèmes potentiels dans le système.
...
Chaque action modifie l'état du système, elle est alors représentée par une arête partant d'un état $e$ vers un état $e'$, on a donc ~: $e \xrightarrow{action} e'$.
...
Ce dernier à été développé par Leslie Lamport \cite{Lamport} dans les années 1990, il est utilisé pour modéliser et vérifier des programmes, notamment sur des systèmes distribués.
...
%0-1 représente une semaine
\begin{figure}[h]
	\centering
	\resizebox{1\textwidth}{!}{
		\begin{tikzpicture}[
			line/.style={very thick},
			model/.style={rectangle, draw=green!50!black, fill=green!20},
			code/.style={rectangle, draw=blue!50!black, fill=blue!20},
			mixed/.style={rectangle, draw=orange!50!black, fill=orange!20},
			write/.style={rectangle, draw=red!50!black, fill=red!20},
			other/.style={rectangle, draw=yellow!50!black, fill=yellow!20}
		]
		
		\draw[line] (0,0.5) -- (0,1.5);
		\draw[line] (22,0.5) -- (22,1.5);
		\draw[line] (0,1) -- (22,1);

		\draw[other] (0.1,0.8) rectangle (0.9,1.2) ;
		%m0
		\draw[model] (1,0.8) rectangle (2.9,1.2) ;
		\draw[mixed] (3,0.8) rectangle (5.9,1.2) ;
		\draw[-] (1,-0.5) -- (5.9,-0.5) node[midway, below] {Mod. zéro} ;
		\draw[-] (1,-0.25) -- (1,-0.5) ;
		\draw[-] (5.9,-0.25) -- (5.9,-0.5) ;
		%m1
		\draw[mixed] (6,0.8) rectangle (8.9,1.2) ;
		\draw[-] (6,-0.5) -- (8.9,-0.5) node[midway, below] {Mod. découplage};
		\draw[-] (6,-0.25) -- (6,-0.5) ;
		\draw[-] (8.9,-0.25) -- (8.9,-0.5) ;
		%m2
		\draw[mixed] (9,0.8) rectangle (10.9,1.2) ;
		\draw[write] (11,0.8) rectangle (11.9,1.2) ;
		\draw[code] (12,0.8) rectangle (12.9,1.2) ;
		\draw[-] (9,-0.5) -- (12.9,-0.5) node[midway, below] {Mod. fail-safe};
		\draw[-] (9,-0.25) -- (9,-0.5) ;
		\draw[-] (12.9,-0.25) -- (12.9,-0.5) ;
		%m3 + maquette
		\draw[mixed] (13,0.8) rectangle (17.9,1.2) ;
		\draw[-] (13,-0.5) -- (17.9,-0.5) node[midway, below] {Mod. final + maquette};
		\draw[-] (13,-0.25) -- (13,-0.5) ;
		\draw[-] (17.9,-0.25) -- (17.9,-0.5) ;
		%vacances
		\draw[other] (18,0.8) rectangle (18.9,1.2) ;
		%rapport
		\draw[write] (19,0.8) rectangle (21.9,1.2) ;
		\draw[-] (19,-0.5) -- (21.9,-0.5) node[midway, below] {Rapport};
		\draw[-] (19,-0.25) -- (19,-0.5) ;
		\draw[-] (21.9,-0.25) -- (21.9,-0.5) ;

		\end{tikzpicture}
	}
	\caption{Frise chronologique. En vert les périodes de modélisation, en bleu les périodes d'implémentation, en orange les périodes mixtes d'implémentation et de modélisation, en rouge les périodes de rédaction et en jaune les périodes diverses.}
	\label{fig:frise}
\end{figure}


La figure \ref{fig:frise} illustre la répartition temporelle des différentes phases de mon stage. Après une période de découverte du sujet, nous avons pu entamer la modélisation du modèle zéro, en alternant entre modélisation et implémentation. 
...
\begin{wrapfigure}{r}{0.3\textwidth}
	...
	\resizebox{0.3\textwidth}{!}{
		...
		(q3) edge[bend left=5] node[midway, above=-1mm, sloped] {\backward,\swFmt{10d}}  (q4)
		(q4) edge[bend left=5] node[midway, below=-1mm, sloped] {\forward,\swFmt{10d}}  (q3)
		...
		(q4) edge[bend left=5] node[midway, above=-1mm, sloped] {\backward,\swFmt{13d}}  (q5)
		(q5) edge[bend left=5] node[midway, below=-1mm, sloped] {\forward,\swFmt{13d}}  (q4)
...
Grâce à ce graphe, nous avons construit une fonction prenant en paramètre la position \bid{i} d'un train, sa direction \dirFmt{d} et la configuration des aiguillages \swFmt{$\sigma$}, et retournant la position du canton suivant \bid{j}~: $\suc{b_i}{d}{\sigma} = \bid{j}$. S'il n'existe pas de successeur, la fonction renvoie une valeur spéciale~: $\nosuc$.
...
\begin{figure}[h]
	...
	\resizebox{0.5\textwidth}{!}{
		...
		\draw[blue] (3.5,0) -- (6.5,0) node[midway,below] {3};

		\draw (7,0) -- (10,0) node[midway,below] {2};
		\fill[green!50!black] (7.5,0.25) circle (1.25mm);
		\fill[red!50!black] (9.5,0.25) circle (1.25mm);
		\end{tikzpicture}
	}
...
\paragraph{} 
Dans les prochaines sections, nous allons aborder différents modèles, du plus simple au plus abouti, et leurs différences afin de comprendre le cheminement qui a mené au modèle final. Il est important de comprendre notre méthode de travail pour mieux appréhender les évolutions futures.
Comme vu plus haut, la modélisation tend à exprimer des comportements au plus proche de la réalité sans en reproduire les détails jugée inutiles.
C'est pourquoi nous faisons évoluer le modèle de manière itérative, ajoutant progressivement de plus en plus d'éléments.
Une fois un modèle établi et son implémentation réalisée, nous pouvons tester son comportement et ses limites pour apporter des ajustements ou préparer le prochain modèle.
Le modèle zéro est le plus basique, reflétant grossièrement le comportement de la maquette, tandis que le modèle final se rapproche davantage de l'implémentation réelle de cette dernière.
...
Par exemple, au départ du canton 3 il est possible de se diriger vers les cantons 1, 2 ou 4 grâce à un montage spécifique~: 
\begin{itemize}
...
Une ressource devient critique lorsqu'elle est convoitée par plusieurs trains, dans notre cas, il s'agit des cantons.
...
\begin{example}[Ressources critiques]
	...
	(ii) On remarque que pour \tid{1}, \bid{2} est après \bid{1}, alors que pour \tid{2}, \bid{2} est avant \bid{1}. Si on résout le problème sur \bid{2} de sorte à ce que \tid{1} soit prioritaire, alors, lorsque \tid{1} libérera \bid{2}, \tid{2} pourra accéder à \bid{1} sans problème.
...
\subparagraph{}
Une seconde problématique de taille a émergé, lors de tests, pendant la réalisation de ce modèle~: le déclenchement des évènements doit avoir lieu suffisamment tôt.
...
\begin{itemize}
	\item Les jetons : Notés T, ils représentent l'accès à une ressource, ici un canton. Il suffit d'incrémenter la valeur du jeton pour libérer cette ressource.
	 On les définit par $T~:~\posFmt{\mathcal{B}} \mapsto \nats$.
...
\end{itemize}
Dans les règles de transition, on représente le régulateur de deux manières, par un R ou par un sextuplet~: $\langle T,W,A,E,{\text Ne},\swFmt{\sigma} \rangle$.
...
\begin{example}
	~\\
	Soient $\tid{1} = \train{\tid{1}}{\bid{3}}{\dirForward}{P}$ et $R = \langle T,W,A,E,{\text Ne},\swFmt{\sigma} \rangle$, on a $E[\tid{1}]~=~[[\wait{4}{1}],\dots]$ le gestionnaire d'évènements associé à \tid{1}.
...
Comme définis précédemment, les gestionnaires sont indexés sur les trains, les évènements du gestionnaire $g_i$ sont donc déclenché par les mouvements du train \tid{i}.
...
\begin{example}[GoodEnding]
	~\\
	Modélisation de l'état initial d'un scénario (voir \ref{sec:goodEnding}) en utilisant le modèle zéro~:
	\\$\trainFmt{\Gamma}~: \{t_1, t_2\}$ avec,
	\begin{itemize}
		\item[] $t_1 = \trainTuple{\square}{1}{*}{[\su{\forward}{3}]}$
		\item[] $t_2 = \trainTuple{\vartriangle}{4}{*}{[\su{\backward}{1}]}$
	\end{itemize}
	\vspace{0.3cm}
	$R~: \langle T,W,A,E,{\text Ne},S \rangle$ avec,	
	\begin{itemize}
		\item[] $T = [0,0,0,0]$
...
\begin{mathpar}
	\inferrule*[left=\ruleDef{Until}]{
		...
		\langle \trainfull{\trainFmt{t_m}}{\posFmt{b_i}}{\dirFmt{d}}{\trainConcat{\su{d}{N}}{P}}, \reg{T}{W}{A}{E}{{\text Ne}}{\swFmt{\sigma}} \rangle
		\\\reduces
		\langle \trainfull{\trainFmt{t_m}}{\posFmt{b_j}}{\dirFmt{d}}{\trainConcat{\su{d}{N}}{P}}, \reg{T}{W}{A.{\tt decr}(\trainFmt{t_m})}{E}{{\text Ne}.{\tt incr}(\trainFmt{t_m})}{\swFmt{\sigma}} \rangle
	}
\end{mathpar}
...
\noindent
La règle \ruleDef{Until} permet de déplacer un train $\trainFmt{t_m} \in \trainFmt{\Gamma}$ sur un canton adjacent, on observe bien que \bid{i} devient \bid{j}, la valeur renvoyée par la fonction \texttt{suc{}}. 
...
Cela règle le second point de la problématique de ce modèle.
...
\begin{mathpar}
	\inferrule*[left=\ruleDef{WaitBefore}]{
			\bufhead{E[\trainFmt{t_m}][{\text Ne}[\trainFmt{t_m}]]} = \wait{j}{x}  \\ \tokenOf{j} \neq x
		}{
			\trainfull{\trainFmt{t_m}}{\posFmt{b_i}}{\dirFmt{d}}{P}, \reg{T}{E}{Ne}{A}{W}{S}
			\\\reduces
			\trainfull{\trainFmt{t_m}}{\posFmt{b_i}}{\dirFmt{d}}{P}, \reg{T}{{\tt pop}(E[\trainFmt{t_m}][{\text Ne}[\trainFmt{t_m}]])}{Ne}{A}{\majtab{W}{(j,x)}{\trainFmt{t_m}}}{S}
		}
\end{mathpar}
...
\begin{mathpar}
	\inferrule*[left=\ruleDef{IncrAfter}]{
			...
			\and
			\tokenOf{j} = x \\ W[(j, x+1)] = \trainFmt{w_{id}} \\ nextWait(\trainFmt{w_{id}},E[\trainFmt{w_{id}}]) = cptAuth
		}{
			\trainfull{\trainFmt{t_m}}{\posFmt{b_i}}{\dirFmt{d}}{P}, \reg{T}{E}{Ne}{A}{W}{S}
			\\\reduces
			\trainfull{\trainFmt{t_m}}{\posFmt{b_i}}{\dirFmt{d}}{P}, \reg{T.{\tt incr}(j)}{{\tt pop}(E[\trainFmt{t_m}][{\text Ne}[\trainFmt{t_m}]])}{Ne}{\majtab{A}{\trainFmt{w_{id}}}{cptAuth}}{W}{S}
		}
\end{mathpar}
...
\begin{mathpar}
	\inferrule*[left=\ruleDef{IncrBefore}]{
			\bufhead{E[\trainFmt{t_m}][{\text Ne}[\trainFmt{t_m}]]} = \incr{j} \\ \tokenOf{j} = x \\ W[(j, x+1)] = \varepsilon
		}{
			\trainfull{\trainFmt{t_m}}{\posFmt{b_i}}{\dirFmt{d}}{P}, \reg{T}{E}{Ne}{A}{W}{S}
			\\\reduces
			\trainfull{\trainFmt{t_m}}{\posFmt{b_i}}{\dirFmt{d}}{P}, \reg{T.{\tt incr}(j)}{{\tt pop}(E[\trainFmt{t_m}][{\text Ne}[\trainFmt{t_m}]])}{Ne}{A}{W}{S}
		}
\end{mathpar}

\begin{mathpar}
	\inferrule*[left=\ruleDef{WaitAfter}]{
			\bufhead{E[\trainFmt{t_m}][{\text Ne}[\trainFmt{t_m}]]} = \wait{j}{x} \\ \tokenOf{j} = x \\ nextWait(\trainFmt{t_m},E[\trainFmt{t_m}]) = cptAuth
		}{
			\trainfull{\trainFmt{t_m}}{\posFmt{b_i}}{\dirFmt{d}}{P}, \reg{T}{E}{Ne}{A}{W}{S} 
			\\\reduces
			\trainfull{\trainFmt{t_m}}{\posFmt{b_i}}{\dirFmt{d}}{P}, \reg{T}{{\tt pop}(E[\trainFmt{t_m}][{\text Ne}[\trainFmt{t_m}]])}{Ne}{\majtab{A}{\trainFmt{t_m}}{cptAuth}}{W}{S}
		}
\end{mathpar}
...
\begin{example}[Atomicité]
	...
	Dans le cas où l'évènement qui doit être traité serait $[incr(7),turn(11,d)]$ il est possible d'avoir l'exécution suivante~:
	\begin{itemize}
...
\begin{wrapfigure}{r}{0.5\textwidth}
	\resizebox{0.5\textwidth}{!}{
		...
		\node[fichier] (moteur) {simulateur};
		\node[fichier] (scenar) [above left=2cm of moteur] {scénarios};
		\node[fichier] (regles) [above right=2cm of moteur] {règles};
		\node[fichier] (graph) [below=2cm of moteur] {graphe};

		\draw[-stealth] (moteur) -- (scenar) node[midway,above,sloped] {charge};
		\draw[-stealth] (moteur) -- (regles) node[midway,above,sloped] {applique};
		\draw[-stealth, dotted] (moteur) -- (graph) node[pos=0.55,below,sloped] {produit};

		\end{tikzpicture}
	}
	\caption{Architecture du simulateur Python}
	\label{fig:archiSimu}
\end{wrapfigure}


Ces petits scripts Python nous permettaient de charger des scénarios de test et de visualiser le comportement du modèle sur un graphe. Les scénarios étant assez simples, il est facile de voir si le comportement attendu est respecté ou non.
...
\paragraph{Raffinement.} Le modèle précédent était très basique, nous avons soulevé plusieurs points d'amélioration, notamment sur l'atomicité des évènements ou avec l'ajout de nouveaux éléments. Nous allons donc façonner ce modèle de manière à corriger ces limitations, en termes d'ajout, les feux de signalisation entreront en jeu.
...
Dans certains cas, cela ne pose aucun problème, cependant, si l'évènement comporte un ordre "wait(-)", alors il est impératif que le train s'arrête pour ne pas provoquer d'accident.

L'introduction des feux de signalisation va nous permettre de gérer ça en remplaçant le système d'autorisation précédent, impossible à maintenir dans ce nouveau cadre.

\paragraph{Buffer.} Un troisième élément fait son apparition dans l'état du système, le buffer, il est utilisé comme canal de communication entre les trains et le régulateur. C'est une file qui stocke les messages des trains en attendant que le régulateur les traite.
...
Ainsi, un message (\trainFmt{$\triangle$},\posFmt{8}) indique au régulateur que le train \trainFmt{$\triangle$}, à un moment donné, est passé par le canton \posFmt{8}. Avec ce nouvel élément, la représentation d'un état du système évolue~: $\langle B, \trainFmt{\Gamma}, R\rangle$.


\paragraph{Trains.} Très peu de changement du côté des trains, seul le programme a été impacté. Il contient désormais la liste des cantons que le train doit parcourir, sans le départ, plutôt que simplement sa destination.
...
Aussi, maintenant que les trains et le régulateur sont découplés, il faut envoyer des messages via le buffer pour déclencher les évènements. Pour cela, la quasi-totalité des règles concernant le train envoie des messages au buffer, excepté \ruleDef{UntilNext}.
...
\begin{example}
	~\\
	Soit un train $\train{\tid{i}}{3}{\dirBackward}{P}$ dont le programme est le suivant~: $P = [\su{\dirBackward}{[4]}, \su{\dirForward}{[3]}]$, un demi-tour est donc effectué sur le canton 4. 
...
Dernière modification, l'intégration des feux de signalisation, notés F. Il sont présent à chaque extrémité d'un canton, pouvant prendre la couleur rouge ou verte, définit par $F~:~\posFmt{\mathcal{B}} \times \{\dirForward,\dirBackward\} \mapsto \{\siggreen, \sigred\}$. Ils permettent un contrôle des trains par le régulateur sans communiquer directement avec eux, ce système remplace les demandes d'autorisation pour avancer. Bien que plus complexe à gérer, il nous permet un plus grand découplage.
Avec toutes ces modifications, la nouvelle représentation du régulateur est la suivante~: $\regM{E}{T}{\swFmt{\sigma}}{W}{G}{H}{F}$.


\paragraph{Évènements.}
Un nouvel évènement est introduit, noté \authOrder, il est la contraction des ordres \texttt{wait} et \texttt{incr}. C'est un ordre particulier car il n'est utile que dans un cas précis, quand l'évènement d'un train contient un ordre \texttt{turn} impliquant un aiguillage qu'il sera le prochain à utiliser, c'est souvent le cas lorsqu'un train effectue un demi-tour. Prenons par exemple le parcours 8,3,7 sur la maquette (figure \ref{fig:grapheMaquette}) l'aiguillage doit être dans l'état "v" puis "d".
...
\begin{example}[GoodEnding]
	~\\
	Modélisation de l'état initial d'un scénario (voir \ref{sec:goodEnding}) en utilisant le modèle découplage~:
	\\$B~: []$ \commentaire{// vide}
	\vspace{0.3cm}
	\\$\trainFmt{\Gamma}~: \{t_1, t_2\}$ avec,
	\begin{itemize}
		\item[] $t_1 = \trainTuple{\square}{1}{*}{[\su{\dirForward}{[2,3]}]}$
		\item[] $t_2 = \trainTuple{\vartriangle}{4}{*}{[\su{\dirBackward}{[2,1]}]}$
	\end{itemize}
	\vspace{0.3cm}
	$R~: \langle T,W,E,\swFmt{\sigma},G,H,F \rangle$ avec,	
	\begin{itemize}
		\item[] $E = \langle g_1, g_2 \rangle$
		\item[] $T = [0,0,0,0]$
		\item[] $\swFmt{\sigma} = [d]$
...
\begin{mathpar}
	\inferrule*[left=\ruleDef{StartEvent}]{
			...
			\langle B, \trainFmt{\Gamma}, \regM{E}{T}{\swFmt{\sigma}}{W}{false}{H}{F} \rangle
			\reduces
			\langle B, \trainFmt{\Gamma}, \regM{E}{T}{\swFmt{\sigma}}{W}{true}{\majtab{H}{\tid{m}}{\bid{i}}}{F} \rangle
		}
\end{mathpar}
...
\begin{mathpar}
	\inferrule*[left=\ruleDef{EndEvent}]{
			...
			\langle B, \trainFmt{\Gamma}, \regM{E}{T}{\swFmt{\sigma}}{W}{true}{H}{F} \rangle
			\reduces
			\langle \pop{B}, \trainFmt{\Gamma}, \regM{\pop{\handlerOf{E}{\tid{m}}}}{T}{\swFmt{\sigma}}{W}{false}{H}{F} \rangle
		}
\end{mathpar}
...
\begin{mathpar}
	\inferrule*[left=\ruleDef{WaitAfter}]{
			...
			\langle B, \trainFmt{\Gamma},  \regM{E}{T}{\swFmt{\sigma}}{W}{\texttt{true}}{H}{F} \rangle
			\reduces
			\langle B, \trainFmt{\Gamma},  \regM{\pop{\head{E[\tid{m}]}[0]}}{T}{\swFmt{\sigma}}{W}{\texttt{true}}{H}{F^\prime} \rangle
		}
\end{mathpar}
...
\paragraph{Limitations.}
Le modèle découplage a plutôt bien répondu aux limitations précédemment évoquées pour le modèle zéro, notamment au niveau du découplage $\Gamma/R$ et de l'atomicité des évènements. Cependant, ce modèle est encore loin d'être parfait et compte de nombreuses imperfections.
...
\paragraph{Régulateur.} Le régulateur a subi de nombreux changements sur ce modèle. Tout d'abord, la perte de la garde, des feux de signalisation et des aiguillages.
...
De cette manière, on simplifie notre modèle de plusieurs façons, plus besoin de stocker tous les couples (canton/jeton) comme avant, ce qui pouvait causer des problèmes pour des réseaux plus grands, mais également en centralisant les informations concernant l'attente des trains. 
...
Le régulateur s'écrit donc maintenant $\langle E, T, W \rangle$.
...
\subparagraph{} Aussi, avec la modification de la file d'attente, il est intéressant de se pencher plus en détail sur l'ordre \wait{j}{x} car il représente bien les dilemmes liés à la modélisation.
Ce dernier nous indique la ressource que le train attend, ainsi, d'un coup d'oeil sur le programme, nous sommes capables de savoir qu'un train \tid{1} avec l'évènement \wait{3}{x} est prioritaire au train \tid{2} avec l'évènement \wait{3}{x+1}.
Cependant, pour l'implémentation, nous avons besoin de connaître la position du train pour le positionner dans la file d'attente, il serait donc plus pratique que $j$ désigne la position d'attente et non la ressource.
...
\begin{warnbox}
	Pour une question de fidélité nous présentons le modèle tel qu'il est, cependant, avec du recul nous avons remarqué qu'il était possible de simplifier le message envoyé par le train en n'envoyant que l'identifiant du train. Dans ce modèle, l'autre information n'est pas utilisée, ce problème est directement lié à notre méthode de travail~: les itérations peuvent introduire des données utiles à un moment, mais superflues par la suite. Dans notre cas, ces données n'ont pas été retirées.
\end{warnbox}
...
\begin{wrapfigure}{r}{0.3\textwidth}
	...
	\resizebox{0.3\textwidth}{!}{
		...
		\path[-stealth] (r) edge[loop above] ();

		\draw[-stealth] (t) -- (r);
		\draw[-stealth] (t)  .. controls (1.5,-0.2) ..  (s);
		\draw[-stealth] (r) -- (s);
		\draw[-stealth] (s) .. controls (1.5,0.2) .. (t);
		\draw[-stealth] (0,0.7) -- (t);
...
\begin{example}[GoodEnding]
		~\\
	Modélisation de l'état initial d'un scénario (voir \ref{sec:goodEnding}) en utilisant le modèle fail-safe~:
	\\$G~: T$
	\vspace{0.3cm}
...
\subparagraph{}
La seconde règle \ruleDef{SignalReq} permet au régulateur d'interagir avec les feux, c'est cette règle qui traite les requêtes de changement d'état des feux.
...
\paragraph{Limitations.}
Ce modèle parvient à dépasser les limites du modèle précédent en offrant une représentation plus fidèle avec d'un côté les méta-données et de l'autre l'état réel du système.
...
De plus, nous allons introduire un nouvel ordre dans le programme des trains, permmetant de corriger le problème de cycle évoqué plus haut. 
...
Actuellement, les gestionnaires d'évènements ne prennent pas en compte les capteurs, ce qui ne permet pas de réagir à leur activation.
...
\paragraph{Évènements.} Comme vu plus haut, les gestionnaires d'évènements ne sont plus indexés sur les trains mais sur les capteurs. Cela implique que les évènements sont donc liés à un capteur, et non plus à un train. Pour être sûr d'avoir compris, comparons le domaine de définition de l'ancien modèle, $\trainFmt{\gamma}$, avec le nouveau, $\mathcal{\posFmt{B}} \times \{\dirForward, \dirBackward\}$. Dans les anciens modèles, les évènements étaient stockés en fonction du train ayant déclenché l'évènement, maintenant c'est l'identifiant du capteur.
...
De plus, la structure des évènements change aussi, l'information du canton lié à l'évènement en cours est déjà présente dans le message~: les évènements sont, de nouveau, des listes d'ordres uniquement. 
...
\begin{wrapfigure}{r}{0.3\textwidth}
	...
	\resizebox{0.3\textwidth}{!}{
		...
		\draw[-stealth] (m) -- (s);
		\draw[-stealth] (s) .. controls (1.5,0.2) .. (t);
		\draw[-stealth] (m) .. controls (1.25,-2.75) .. (r);
		\draw[-stealth] (r) .. controls (2,-2.75) .. (m);
		\draw[-stealth] (0,0.7) -- (t);
...
Aussi, lorsque le régulateur commence à traiter des évènements, il le fait jusqu'à ce que le buffer soit vide.
...
\begin{example}[GoodEnding]
		~\\
	Modélisation de l'état initial d'un scénario (voir \ref{sec:goodEnding}) en utilisant le modèle final~:
	\\$G~: T$
	\vspace{0.3cm}
	\\$B_t~: [(4,\dirBackward)]$
	\vspace{0.3cm}
	...
	\\$\trainFmt{\Gamma}~: \{t_1, t_2\}$ avec,
	\begin{itemize}
		\item[] $t_1 = \trainTupleMF{\square}{1}{*}{[\su{\dirForward}{[2,3]}]}{1}$
		\item[] $t_2 = \trainTupleMF{\vartriangle}{4}{*}{[\su{\dirBackward}{[2,1]}]}{4}$
	\end{itemize}
	\vspace{0.3cm}
	$R~: \langle T,W,E,H,CP \rangle$ avec,	
	\begin{itemize}
		\item[] $E = [\dots,(3,\dirForward)~:~[\turnOrder{5}{v}, \incr{2}], (4,\dirBackward)~:~[\wait{2}{1}],\dots]$
		\item[] $T = [0,0,0,0]$
		\item[] $W = \{\}$
...
De nouvelles règles concernant les déplacements de trains sont introduites dans ce modèle, en accord avec les objectifs évoqués plus haut. La règle \ruleDef{Until} change peu, on utilise maintenant la fonction \texttt{tsuc} plutôt que \texttt{suc}. De cette manière, on peut savoir si le successeur est un canton ou non.
...
\begin{figure}[h]
	...
	\caption{Avant/Après la règle ExitBlock. En pointillé, la position du train selon le régulateur.}
...
\begin{figure}[h]
	...
	\caption{Avant/Après la règle EnterSwitch.}
	\label{fig:exEnterSwitch}
\end{figure}

La règle \ruleDef{EnterSwitch} est presque similaire à la précédente. Tout d'abord, on note la disparition de la contrainte sur le feu et de l'envoi de message, le train se trouvant sur un aiguillage, aucun feu de signalisation ou capteur n'est présent.
Enfin, pour utiliser cette règle, nous avons besoin que le train se trouve sur un aiguillage, d'où la contrainte $\texttt{aig(\bid{i})} = true$. Dernière règle de cette section, nous allons voir comment un train passe d'un aiguillage à un canton.
...
\begin{figure}[h]
	...
	\caption{Avant/Après la règle EnterBlock.}
	\label{fig:exEnterBlock}
\end{figure}

\noindent
Cette règle "signe" la resynchronisation du train avec le reste du système, on observe que $pos$ est égale à $tpos$. C'est aussi le retour de l'envoi de message, le train se trouvant dans un canton, il a donc franchit le capteur de début de canton, $\langle\bid{j},\dirFmt{d^{-1}}\rangle$. 

\paragraph{Limitations.} Bien que ce modèle soit le plus proche de la réalité, il se place dans un environnement hostile puisque nous n'imposons pas de garantie sur le délai de traitement des messages. Or, dans la réalité, le traitement des informations et bien plus rapide que le déplacement des trains, beaucoup de mécanismes présents dans ce modèle n'ont donc pas lieu d'être.
...
\paragraph{Introduction.} \TLA est un langage de spécification formelle développé par Leslie Lamport , très populaire au sein de la communauté des systèmes distribués. Il est principalement utilisé pour faire de vérification de programmes, c'est-à-dire prouver que le programme respecte certaines propriétés.
...
\paragraph{Spécification.} Les spécifications \TLA telles que nous allons les utiliser comportent plusieurs éléments clé, comme des \emph{variables}, des \emph{propriétés temporelles} et des \emph{actions}, nous verrons cela plus en détail par la suite.
Ces éléments doivent être rédigés en utilisant la syntaxe formelle de l'outil, qui repose grandement sur des expressions mathématiques et logiques. Ce langage n'est pas un langage de programmation, il n'est pas possible d'écrire de boucle et les structures de données sont "limitées", mais il est quand même possible de définir des fonctions. 
...
\begin{figure}[h]
	\caption{Initialisation des variables dans le modèle fail-safe}
	\label{fig:initTLA}
\end{figure}
...
On sépare ces clauses en deux catégories, les conditions nécessaires à l'action, équivalentes à nos prémisses dans les règles, et les clauses modifiant l'état, équivalentes à nos conclusions. Ces dernières sont facilement identifiables grâce à l'opérateur prime ($^\prime$), à noter qu'une variable ne subissant pas de modification sera notée \texttt{UNCHANGED}.

\subparagraph{} Enfin, les propriétés temporelles vont déterminer si l'implémentation de notre modèle a le bon comportement. Pour cela, on définit deux propriétés~: \textit{safety} et \textit{liveness}.
...
\noindent
Pour nous aider à formuler ces propriétés, \TLA inclut deux opérateurs temporels~:
\begin{itemize}
...
Pour indiquer cela à \TLA, nous devons utiliser la \textit{weak fairness}, une condition qui stipule que si une action peut être exécutée, alors elle sera finalement exécutée. Dans la spécification, cette condition est représentée par \texttt{WF\_<<\dots>>(Next)}.
...
\paragraph{} Jusqu'à présent, nous avons considéré que les programmes des trains et du régulateur étaient créés par un humain. Cependant, il est possible d'automatiser ce processus, c'est ce que nous allons voir dans cette section.
...
\paragraph{}Nous avons d'abord identifié deux types de trajets~:
\begin{itemize}
...
\begin{example}
	~\\
	Soit un chemin non-élémentaire $c$ dont les trains \tid{a} et \tid{b} partagent une ressource critique $crit$ et dont les cantons parcourus sont respectivement $P_a = [a,crit,a^\prime]$ et $ P_b = [b,crit,b^\prime]$; avec $a,a^\prime,b,b^\prime$ des liste de cantons.
...
\paragraph{}
Dans le but de tester notre fonction de composition, nous avons développé un ensemble de scripts Python pour automatiser la création et la vérification de chemins composés.
...
\begin{figure}[h]
	...
	\end{tikzpicture}
	\caption{Workflow des tests. Les rectangles noirs sont des programmes. Les rectangles bleus arrondis sont des fichiers, y compris les fichiers générés remplis en bleu.}
	\label{fig:toolchain}
\end{figure}
...
On observe que les compositions de deux chemins élémentaires se déroulent sans problème pour nos deux modèles \footnote{Les expériences n'ont été menées que sur le modèles 2 et le modèle final, car ce sont les seuls à être doté d'une fonction de composition.} avec 0 erreur à signaler.
...
Pour la composition à trois chemins, nous avons mené l'expérience sur un échantillon réduit pour avoir une intuition sur les performances de la fonction de composition. Ces tests avaient été menés dans le cadre de l'écriture d'un article, la deadline étant courte, nous n'avions pas assez de temps pour tester sur tous les cas possibles.
Les 91 cas menant à une erreur ont été vérifiés manuellement sur une machine personnelle et n'ont produit aucune erreur, nous suspectons un léger problème dans l'environnement de test (un fichier qui n'est pas à jour, une version obsolète, etc).
Pour le modèle final, nous n'avons pas eu le temps de mener les tests à trois chemins.


\begin{warnbox}
	Les scripts utilisés pour les expériences sont disponibles sur Zenodo à l'adresse suivante~: \url{https://doi.org/10.5281/zenodo.15761521}.
	Le code n'a pas vocation à être utilisé en production, il a pour but d'expérimenter. De plus, il est à replacer dans un contexte de recherche et de deadline serrée.
\end{warnbox}
...
\paragraph{Maquette.} Une fois les principaux objectifs atteints, nous avons pu nous pencher sur des problèmes secondaires, comme la création d'une "maquette de la maquette".
...
\begin{figure}[h]
	...
	\begin{tikzpicture}
		...
		\draw[-stealth] (rasp) edge[bend left=0.5cm] node[above,sloped] {I2C} (ard);
		\draw[-stealth] (ard) edge[bend left=0.5cm] node[above,sloped] {I2C} (rasp);
		\draw[-] (ard) -- (f1);
		\draw[-] (ard) -- (f2);
		\draw[-] (ard) -- (a1);
		\draw[-] (ard) -- (f3);
...
\paragraph{Article.}
Nous avons rédigé un article pour partager notre travail sur le modèle fail-safe, sur la fonction de composition et sur les expériences menées. Malgré des délais très courts, ce dernier a été soumis dans les temps à la conférence \texttt{RSSRail 2025}, mais il n'a pas été retenu.
La relecture par les pairs a été une expérience enrichissante, nous avons reçu des retours constructifs qui nous aiderons à améliorer l'article pour une future soumission.
...
Il a ensuite fallu créer des modèles successifs, chaque fois plus proches de la réalité, en intégrant des éléments comme les capteurs de voie ou les feux de signalisation.
...
Cependant, notre méthodologie a parfois montré des faiblesses, notamment en ce qui concerne les données superflues. Une donnée inutile dans le modèle, vestige du modèle précédent, peut complexifier la compréhension du système. Aussi, de par sa nature itérative, si un problème n'a pas été détecté dans un modèle antérieur, il se propagera sur les suivants.
...
\section{GoodEnding}
\label{sec:goodEnding}
Le scénario \textit{GoodEnding} est issue de nos scénarios de test, il est le plus basique, mais comprend déjà un bon nombre d'interactions.
...
\begin{figure}[H]
	...
	\resizebox{0.5\textwidth}{!}{
		...
		\node[vertex] (q1)	  				  {1};
		\node[vertex] (q2) [right=40mm of q1] {2};
		\node[vertex] (q3) [right=40mm of q2] {3};
		\node[vertex] (q4) [above=20mm of q3] {4};

		\draw[->] 
		(q1)  edge[bend left=5]  node[midway, above=-1mm, sloped] {\forward} (q2)
		(q2) edge[bend left=5] node[midway, below=-1mm, sloped] {\backward}  (q1)
		(q2)  edge[bend left=5]  node[midway, above=-1mm, sloped] {\forward,\swFmt{5d}} (q3)
		(q3) edge[bend left=5] node[midway, below=-1mm, sloped] {\backward,\swFmt{5d}}  (q2)
		(q2)  edge[bend left=5]  node[midway, above=-1mm, sloped] {\forward,\swFmt{5v}} (q4)
		(q4) edge[bend left=5] node[midway, below=-1mm, sloped] {\backward,\swFmt{5v}}  (q2)

		;
		\end{tikzpicture}
	}
	\caption{Graphe modélisant le réseau du scénario \textit{GoodEnding}}
	\label{fig:goodEnding}
\end{figure}
