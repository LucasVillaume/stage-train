...
\begin{wrapfigure}{r}{0.35\textwidth}
	...
	\resizebox{0.3\textwidth}{!}{
		...
		\draw[->] (q0.south) -- (q1.north) node[midway, right] {Pièce insérée};
		\draw[->] (q1.south) -- (q2.east)  node[midway, right] {Choix thé};
		\draw[->] (q1.south) -- (q3.west)  node[midway, right] {Choix café};

		\draw[->] (q2.north) .. controls (0,0) .. (q0.west) node[midway, right] {Thé prêt};
		\draw[->] (q3.north) .. controls (8,0) .. (q0.east) node[midway, right] {Café prêt};
...
\begin{wrapfigure}{r}{0.3\textwidth}
	...
	\resizebox{0.3\textwidth}{!}{
		...
		\draw[edge] (q1.west)  -- (q3.east)  node[midway, above, sloped] {9d/10v};
		\draw[edge] (q2.west)  -- (q3.east)  node[pos=0.3, above, sloped] {9v/10v};
		\draw[edge] (q4.west)  -- (q3.east)  node[midway, above, sloped] {10d};
		\draw[edge] (q3.south) -- (q8.west)  node[midway, above, sloped] {11v};
		\draw[edge] (q3.south) -- (q7.north) node[midway, above, sloped] {11d};
		\draw[edge] (q7.east)  -- (q5.west)  node[midway, above, sloped] {12d};
		\draw[edge] (q7.east)  -- (q6.south) node[midway, above, sloped] {12v};
		\draw[edge] (q5.north) -- (q4.south) node[midway, above, sloped] {13d};
		\draw[edge] (q6.north) -- (q4.south) node[midway, above, sloped] {13v};
		\end{tikzpicture}
	}
...
Comme expliqué dans l'introduction, ce sont les règles de transitions qui vont apporter du mouvement à notre modèle, permettant de modifier son état règles après règles.
...
\noindent
Les règles doivent pouvoir traiter les ordres des événements, on pourrait s'attendre à avoir une règles \ruleDef{Wait} et une règle \ruleDef{Incr}. Cependant, nous avons besoin de comportements plus fin, nous allons voir ça dès maintenant. 

\begin{mathpar}
	\inferrule*[left=\ruleDef{WaitBefore}]{
			\bufhead{E[\trainFmt{t_m}][{\text Ne}[\trainFmt{t_m}]]} = wait(j, y) \\ J[j] \neq y
		}{
			\trainfull{\trainFmt{t_m}}{\posFmt{b_i}}{\dirFmt{d}}{P}, \reg{J}{E}{Ne}{A}{W}{S}
			\\\reduces
			\trainfull{\trainFmt{t_m}}{\posFmt{b_i}}{\dirFmt{d}}{P}, \reg{J}{{\tt pop}(E[\trainFmt{t_m}][{\text Ne}[\trainFmt{t_m}]])}{Ne}{A}{\majtab{W}{(j,y)}{\trainFmt{t_m}}}{S}
		}
\end{mathpar}
...
\begin{mathpar}
	\inferrule*[left=\ruleDef{IncrAfter}]{
			...
			\and
			J[j] = x \\ W[(j, x+1)] = \trainFmt{w_{id}} \\ nextWait(\trainFmt{w_{id}},R) = cptAuth
		}{
			\trainfull{\trainFmt{t_m}}{\posFmt{b_i}}{\dirFmt{d}}{P}, \reg{J}{E}{Ne}{A}{W}{S}
			\\\reduces 
...
Ainsi, la première permet d'inscrire la train à la file d'attente~: $\majtab{W}{(j,y)}{\trainFmt{t_m}}$, tandis que la seconde incrémente le jeton et mets à jour les autorisations~: $\majtab{A}{\trainFmt{w_{id}}}{cptAuth}$.
Si l'on regarde les prémisses de la règles d'incrémentation, on remarque que l'on fait appel à la fonction $nextWait(\trainFmt{w_{id}},R)$

%Paragraphe "imperfection"
%\subparagraph{téléportation des trains de canton à canton}
%\subparagraph{Event non atomique, donc attention à l'ordre }
...
\paragraph{Python.} Décrire le rôle du python
...
\todo{expliquer ce que fait grossièrement le code, l'architecture et les "features" (chargement de scénarios et visualisation sur graph) Et commentaire sur le besoin de passer à TLA}
