...
\todo{ Comparaison avec les travaux antérieurs }
...
The waiting field is a function $W: \blocks \times \Nat \to \trains_0$. It states that block must be protected. More precisely, suppose that for some block $pid$, some train $tid$ tries to enter the block. If $W(pid,T(pid)) = tid$, then, the train shall wait the token is released. The value $W(pid, n) = 0$ means the train did not reach the current position $pid$. 

The history field $H: \trains \to \blocks$ gives the last position of each train recorded  by the regulator. \todo{A vérifier.}
