...
The regulator is characterized as follows. First, it has a general program, next mentioned as $E$. The program contains the plan of the regulator. The regulator will follow its rule. A program is given as a sequence of statements. Those follow the grammar:

\begin{grammar}[Regulator's rules][h][regulateur]
    \firstcasesubtil{stmt}{ev(id, n) : atoms}{}
    \firstcasesubtil{atoms}{incr(j)}{}
    \otherform{att(j, value_{j})}{}
    \otherform{turn(id_{switch}, state)}{}
    \otherform{auth}{}
    \otherform{atoms \gralt \varepsilon}{}
\end{grammar}
