...
\begin{abstract}
	...
	%Both switches and traffic lights are controlled by a regulator upon train events. 
...
\paragraph{Model.}
Before delving further in the details, we introduce a real-world N-scale train model (\ref{fig:maquette}), which shaped our approach and was used to test our results. This model has 8 blocks, 5 switches. Each block is bidirectional with a signal at each end. %Blocks size goes from 25cm to 70cm.
...
The train network is described as follows. We consider a set of $n$ blocks: $\blocks = \{ \bid{1}, \bid{2}, \ldots, \bid{n}\}$. Blocks are oriented: there is a $\dirForward$ (forward) end and a $\dirBackward$ (backward) one. Let $\directions = \{\dirForward, \dirBackward\}$ be the set of directions.  Then, we consider a set of $m$ trackworks\footnote{For the sake of simplicity, in this paper, we only consider switches. Our work can  be extended to crossings and double slip switches (e.g. crossings have only one internal state and double slip switches have four states).}: $\turnouts = \{\sid{1}, \ldots, \sid{m}\}$. Trackworks are characterized by an internal state: switches can be in direct or deviate position (next denoted '\direct' and '\deviate'). A \emph{network configuration} is a function $\sigma: \turnouts \to \{\direct, \deviate\}$. Let $\networkConf$ denotes the set of all network configurations. For instance, for two turnouts $\sid{1}$ and $\sid{2}$, the function $\sid{1} \mapsto \deviate, \sid{2} \mapsto \direct$ states that switch $\sid{1}$ (resp. $\sid{2}$) is deviate (resp. direct).
% The latter function is also described via an array  $[\deviate, \direct]$ giving states in trackwork order.
...
%We note $\incrToken{T}{\bid{i}}$ the list of tokens identical to $T$, except for $\tokenOf{\bid{i}}$, which is incremented.
...
Given a routing program $E$, we note $\handlerOf{E}{\tid{i}}$ the list of handlers of train $\tid{i}$, $\head{\handlerOf{E}{\tid{i}}} = \handlerOf{E}{\tid{i}}[1]$ the first handler of the list. Given a handler $h = \tuple{\bid{i}, A}$, let  $\actionsOf{h} \triangleq A$, $\blockOf{h} \triangleq \bid{i}$. When $h = \tuple{\bid{i}, a \cdot A}$ is the head of a sequence $H = h \cdot H'$,  $\nextAct{H} \triangleq \tuple{\bid{i}, A} \cdot H'$. %}{} \triangleq H'$ .  %(resp. \blockOf{h}) the list of action (resp. the block id) of $h$.
% We note $\nextAct{\handlerOf{E}{\tid{i}}}$ the routing program $E$ where the first event handler of train $\tid{i}$ is replaced with $\handler{\blockOf{\head{\handlerOf{E}{\tid{i}}}}}{\buftail{\actionsOf{\head{\handlerOf{E}{\tid{i}}}}}}$, i.e. where the first action of the first handler of train \tid{i} is removed.
...
% We suppose all train tuples in $\trainSeq$ have a different identifier. 
...
%In the previous section, we introduced our model. In this section, 
...
We enrich our model with three elements: a guard and two buffers. They ensure agents' autonomy. %, which goal is to schedule agents (for instance, it ensures the regulator acts atomically); and a communication buffer, which stores pending events until the regulator treats them. 
...
%		\node[draw, right=of regulator] (signals) {Signals update (\guardS)};
%		
...
States of the system are 4-tuples from $Q\times \bufTrainSet \times \bufSigSet \times \modelSet$. Our reduction relation is a binary relation on system states, noted $\reduces$: $S_1 \reduces S_2$ means that state $S_1$ can change to $S_2$ (note that this relation is not a function: $S_1$ can reduce to multiple states, due to non-determinism). We define $\reduces$ as the least relation induced by the inference rules presented hereafter.
...
\begin{mathpar}
	\inferrule*[left=\ruleDef{Start}]{
		...
		\hspace{3mm} \redTuple{\guardT}{\bufTrain}{\bufSig}{\stateTuple{\trainSeq{}\cup\{\trainTuple{\tid{m}}{\bid{i}}{d}{P}\}}{\regulator}{\switches}{\signals}} \\
		\reduces
		\redTuple{\guardT}{\push{\bufTrain}{\tid{m}}}{\bufSig}{\stateTuple{\trainSeq{}\cup\{\trainTuple{\tid{m}}{\bid{i}}{d^\prime}{P}\}}{\regulator}{\switches}{\signals}}
	}
\end{mathpar}
...
\begin{mathpar}
	...
	\reduces
	\redTuple{\guardT}{\push{\bufTrain}{\tid{m}}}{\bufSig}{\stateTuple{\trainSeq{}\cup\{\trainTuple{\tid{m}}{\bid{i}}{\dirStop}{\emptyTrainProg}\}}{\regulator}{\switches}{\signals}}
	}
\end{mathpar}
...
\begin{mathpar}
	\inferrule*[left=\ruleDef{Until}] {
		...
		\redTuple{\guardT}{\bufTrain}{\bufSig}{\stateTuple{\trainSeq{}\cup\{\trainTuple{\tid{m}}{\bid{i}}{d}{\trainConcat{\su{d}{\posFmt{N}}}{P}}\}}{\regulator}{\switches}{\signals}} \\
		\reduces		
		\redTuple{\guardS}{\push{\bufTrain}{\tid{m}}}{\bufSig}{\stateTuple{\trainSeq{}\cup\{\trainTuple{\tid{m}}{\bid{j}}{d}{\trainConcat{\su{d}{\posFmt{N'}}}{P}}\}}{\regulator}{\switches}{\signals}}
	}
\end{mathpar}
...
\begin{mathpar}
	\inferrule*[left=\ruleDef{UntilNext}] {
		...
		\redTuple{\guardT}{\bufTrain}{\bufSig}{\stateTuple{\trainSeq{}\cup\{\trainTuple{\tid{m}}{\bid{i}}{d}{\trainConcat{\su{d}{\posFmt{N}}}{P}}\}}{\regulator}{\switches}{\signals}} \\
		\reduces		
		\redTuple{\guardS}{\bufTrain}{\bufSig}{\stateTuple{\trainSeq{}\cup\{\trainTuple{\tid{m}}{\bid{j}}{d}{P}\}}{\regulator}{\switches}{\signals}}
	}
\end{mathpar}
...
Then, the regulator lookup the first event on the buffer, and executes all the rules in the event handler associated with this event. All actions are treated in order, and when the list of remaining actions is empty, the handling terminates and the event is removed of the buffer. We introduce 6 reduction rules for the 4 possible kinds of orders (\incr{\bid{i}} and \wait{\bid{i}}{n} have two rules).

Rule \ruleRef{Turn} triggers when a \turn{\sid{i}}{s} is encountered in the event handler. It updates the state of the targeted switch.
\begin{mathpar}
	\inferrule*[left=\ruleDef{Turn}]{
		\bufhead{\bufTrain} = \tid{i}
		\and
		\actionsOf{\head{\handlerOf{E}{\tid{i}}}} = \regConcat{\turn{\sid{i}}{s}}{A^\prime}
	}{
	\redTuple{\guardR}{\bufTrain}{\bufSig}{\stateTuple{\trainSeq}{\regTuple{E}{T}{W}}{\switches}{\signals}}
	\\\reduces
	\redTuple{\guardR}{\bufTrain}{\bufSig}{\stateTuple{\trainSeq}{\regTuple{\nextAct{\handlerOf{E}{\tid{i}}}}{T}{W}}{\updateSwitch{\switches}{\sid{i}}{s}}{\signals}}
	}
\end{mathpar}
...
\begin{mathpar}
	\inferrule*[left=\ruleDef{Auth}] {
		\bufhead{\bufTrain} = \tid{i}
		\and
		\bufhead{E[\tid{i}]} = \handler{\bid{src}}{\regConcat{\auth}{A^\prime}}
		\\
		\nextWait{\buftail{E[\tid{i}]}} = \bid{\text{stop}}
	}{
			\redTuple{\guardR}{\bufTrain}{\bufSig}{\stateTuple{\trainSeq}{\regTuple{E}{T}{W}}{\switches}{\signals}}
			\\\reduces
			\redTuple{\guardR}{\bufTrain}{\push{\bufSig}{\tuple{\bid{\text{stop}}, \sigred}}}{\stateTuple{\trainSeq}{\regTuple{\nextAct{\handlerOf{E}{\tid{i}}}}{T}{W}}{\switches}{\signals}}
	}
\end{mathpar}
...
Finally, when the handler of the pending event has no action leftover, the regulator terminates its handling: it removes the (now empty) head of $\handlerOf{E}{\tid{i}}$, the head of the buffer, and returns the guard to state $\guardT$.

\begin{mathpar}
	\inferrule*[left=\ruleDef{EndEvent}]{
		\bufhead{\bufTrain} = \tid{i}
		\and
		\actionsOf{\head{\handlerOf{E}{\tid{i}}}} = \emptyList
	}{
		\redTuple{\guardR}{\bufTrain}{\bufSig}{\stateTuple{\trainSeq}{\tuple{E, T, W}}{\switches}{\signals}}
		\\\reduces
		\redTuple{\guardS}{\buftail{\bufTrain}}{\bufSig}{\stateTuple{\trainSeq}{\tuple{\popHandlerHead{E}{\tid{i}}, T, W}}{\switches}{\signals}}
	}
\end{mathpar}
...
\begin{itemize}
	\item if there is a train occupying block $\bid{i}$, then signals in neighbour blocks leading to \bid{i} are turned to \sigred (computed using \trainSeq and \switches);
...
where variables is the tuple of all variables. The intended meaning is that the initial configuration is specified in \mintinline{C}{Init} and steps follow the \mintinline{C}{Next} formula.  Without the weak fairness condition \mintinline{C}{WF_variables(Next)},  nothing forces a rule to be applied. As a consequence, an ongoing train could never reach its next block. The weak fairness condition prevents this incorrect simulation. 
...
A \emph{safe step} is a global routing  coming with a network configuration $\switches$ and a direction map $d_m: \trains \to \directions$ such that:
\begin{itemize}
\item there are no critical resources between any two routes,
\item $\switches$ and $d_m(\tid{i})$ are witnesses of all transitions in $\vec{x}^\tid{i}$. 
...
Now, let us turn to the definition of $E$. For any train $\tid{i} \in \trains$, we define $A= (\emptyList)_{x \in \vec{x}^\tid{i} }$. Then, set  $E[\tid{i}] = \langle d_m(\tid{i}), A[x_1 \leftarrow \langle \incr{x_1^\tid{i}}] \rangle]$. 
...
Second, we proceed by updates of $E$ and $\switches$. At first, we set $E' = E$ and $\switches' = \switches$. We begin to append to $E'$, for any train, all the handlers within $S_k$. To each train $\tid{i}$, we had for its first step (within $X_k$, seen within the composition): an $\auth$ order in $E'[\tid{i}]$. 

Consider some turnout $\sid{i}$ such that $\sid{i} \in \throughS(x_n, d_{mk}(\tid{i}), \switches_k, x_{n+1})$ with $\vec{x}$ the route corresponding to train $\tid{i}$ in $X_k$. In other words, the turnout is crossed by train $\tid{i}$ at step $n$ of $\vec{x}$ within $X_k$. Suppose there is a step $j < k$, a train $\tid{m}$ such that $\sid{i} \in \throughS(y_m, d_{mj}(\tid{m}), \switches_j, y_{m+1})$ with $\vec{y}$ being the route of train $\tid{m}$ at step $X_j$. If $\switches_j(\sid{i}) = \switches_k(\sid{i})$, we skip this turnout. Otherwise, if $\tid{i} = \tid{m}$, call $p$ the position within $E'$ corresponding to block $x_{n+1}$, we append in $E'$ at that position the order $\auth$. Otherwise, call $p$ the position in the map $E'$ corresponding to the atoms related to the block $y_{m+1}$. We append to that position the order $\turn{\sid{i}}{d_{mk}(\sid{i})})$. In other words, we append to the last step involving the turnout the order for the new position. 
...
\begin{figure}
	...
	\end{tikzpicture}
	\caption{Overview of the toolchain. Black rectangles are programs. Blue rounded rectangles are files, included generated files filled in blue. The main script uses a json file with safe steps (c.f. Section~\ref{sec:composition}) to produce composed routes (calling the script \texttt{compose.py}). Routes are then converted in TLA with \texttt{to\_tla.py}, producing a file \texttt{route.tla}. \texttt{tlc}, a TLA model checker, then uses the route and the model to check for correctness, producing log files.}
	\label{fig:toolchain}
\end{figure}
...
\paragraph{Contributions.}
This paper introduces three main contributions. First we presented a formal model for railway network. Our model simplifies multiple elements from real life networks, but nonetheless keeps non-trivial elements, such as trackworks and signals.
...
Our implementation, including both the TLA model and the Python composition is available on \url{https://github.com/LucasVillaume/stage-train}.\todo{Remplacer Zenodo si on a le temps}.
