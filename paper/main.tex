...
\newcommand{\guardS}{\guardFmt{S1}}
\newcommand{\guardSS}{\guardFmt{S2}}
...
\paragraph{Model.}
Overall, states of our model are tuples $\tuple{\trainSeq, \regulator, \switches, \signals}$ where $\trainSeq$ is a sequence of train tuples, $\regulator$ a regulator tuple, $\switches$ a network configuration (indicating trackworks states) and $\signals$ a signals state function.
...
Essentially, this guard is the state machine shown in Figure~\ref{fig:state_machine_guard}. The four states of the guard are \guardR, \guardS, \guardSS, and \guardT.
...
\begin{figure}
	...
	\begin{tikzpicture}
		...
		\node[draw, below right=of trains] (regulator) {Regulator reaction (\guardT)};
		\node[draw, right=of regulator] (signals2) {Signals update (\guardSS)};
		...
		\draw[-stealth] (regulator) to (signals2);
		\draw[-stealth] (signals2) to[bend right=10] (trains);
	\end{tikzpicture}
