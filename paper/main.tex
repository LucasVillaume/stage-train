...
\newcommand{\setSignalsTo}[3]{\signals.{\tt set\_to}(#1, #2, #3)}
...
We now introduce two rules to update signals. In the regulator rules, we push signal updates request in the signal buffer. Signal rule \ruleRef{SignalReq} read those requests and perform appropriate updates. When all requests are dealt with, a final rule \ruleRef{SignalSfty} updates all signals based on safety rules (e.g. even if there is no rule to explicitly turn red signals leading to occupied blocks, the safety rules automatically turn it red).
...
\begin{mathpar}
	\inferrule*[left=\ruleDef{SignalReq}]{
		...
		\\\reduces
		\redTuple{\guardS}{\bufTrain}{\buftail{\bufSig}}{\stateTuple{\trainSeq}{R}{\switches}{\setSignalsTo{\bid{id}}{c}{\switches}}}
	}
\end{mathpar}

\begin{mathpar}
	\inferrule*[left=\ruleRef{SignalSfty}] {
	}{
		\redTuple{\guardS}{\bufTrain}{\emptyList}{\stateTuple{\trainSeq}{R}{\switches}{\signals}}
	}
\end{mathpar}
