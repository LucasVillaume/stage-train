...
\begin{mathpar}
	\inferrule*[left=\ruleDef{Start}]{
		...
		\reduces
		\redTuple{\guardT}{\bufferFmt{B}}{\stateTuple{\trainSeq{}\cup\{\trainTuple{\tid{id}}{\bid{id}}{d^\prime}{P}\}}{\regulator}{\switches}{\signals}}
	}
\end{mathpar}
...
Rule application induce a relation between configuration:  $\gamma \reduces \gamma'$ if $\gamma'$  if there is a rule for which $\gamma$ is the initial state and $\gamma'$ the conclusion. The reflexive-transitive closure of $\reduces$ is denoted $\reduces^*$, that is $\gamma \reduces^* \gamma'$ iff $\gamma = \gamma_0 \reduces \cdots \gamma_n = \gamma'$. The value $n = 0$ corresponds to $\gamma = \gamma'$. 

A normal form of a configuration $\gamma$ is a configuration $\gamma'$ such that $\gamma \reduces^* \gamma'$ and there is no $\gamma''$ with $\gamma' \reduces \gamma''$. In other words, starting from $\gamma$, the computation may end on $\gamma'$. We note $\gamma \reduces^! \gamma'$ this relationship.

Given a configuration of the shape  $\gamma = \redTuple{x}{B}{\stateTuple{\trainSeq{}}{\regulator}{\switches}{\signals}}$ and a train $\tid{id}$, 
\begin{itemize}
\item   we say that it has no jobs if  $\trainSeq{}[\tid{id}].P = \emptyTrainProg$;
\item its source is $\mathtt{src}(\gamma, \tid{id}) = \trainSeq{}[\tid{id}].pid$;
\item its destination is $\mathtt{dst}(\gamma, \tid{id}) = pid \in \blocks $ if either
\begin{itemize}
