...
Similarly, the regulator communicates with signals with a \emph{signal buffer} $B_s$, which is a list of tuple $\tuple{\bid{id}, c}$, indicating that both signals of block $\bid{id}$ should be turned to color $c$. \todo{mise en page}
...
\begin{mathpar}
	\inferrule*[left=\ruleDef{UntilNext}] {
		...
		\reduces		
		\redTuple{\guardS}{\bufferFmt{B}}{\stateTuple{\trainSeq{}\cup\{\trainTuple{\tid{id}}{\bid{id^\prime}}{d}{P}\}}{\regulator}{\switches}{\signals}}
	}
\end{mathpar}
...
\begin{mathpar}
	\inferrule*[left=\ruleDef{WaitBefore}] {
		...
		\redTuple{\guardR}{\bufferFmt{B}}{\stateTuple{\trainSeq}{\tuple{E, T, W}}{\switches}{\signals}}
		\reduces
		\redTuple{\guardR}{\bufferFmt{B}}{\stateTuple{\trainSeq}{\tuple{\substHandlerHead{E}{\tid{id}}{A^\prime}, T, W\cup\tuple{\bid{src}, \bid{critical}, n, \tid{id}}}}{\switches}{\signals}}
	}
\end{mathpar}
...
\begin{mathpar}
	\inferrule*[left=\ruleDef{IncrAfter}] {
		...
		\redTuple{\guardR}{\bufferFmt{B}}{\stateTuple{\trainSeq}{\tuple{E, T, W}}{\switches}{\signals}}
		\\\reduces
		\redTuple{\guardR}{\bufferFmt{B}}{\stateTuple{\trainSeq}{\tuple{\substHandlerHead{E}{\tid{id}}{A^\prime}, T[\bid{critical} \leftarrow T(\bid{critical})+1], W\setminus\{\tuple{\bid{src}, \bid{critical}, \tokenOf{\bid{critical} + 1}, \tid{waiting}}\}}}{\switches}{\signals}}
	}
\end{mathpar}
...
\begin{mathpar}
	\inferrule*[left=\ruleDef{IncrBefore}] {
		...
		\redTuple{\guardR}{\bufferFmt{B}}{\stateTuple{\trainSeq}{\tuple{E, T, W}}{\switches}{\signals}}
		\\\reduces
		\redTuple{\guardR}{\bufferFmt{B}}{\stateTuple{\trainSeq}{\tuple{\substHandlerHead{E}{\tid{id}}{A^\prime}, T[\bid{critical} \leftarrow T(\bid{critical})+1], W}}{\switches}{\signals}}
	}
\end{mathpar}
...
\begin{mathpar}
	\inferrule*[left=\ruleDef{WaitAfter}] {
		...
		\redTuple{\guardR}{\bufferFmt{B}}{\stateTuple{\trainSeq}{\tuple{E, T, W}}{\switches}{\signals}}
		\reduces
		\redTuple{\guardR}{\bufferFmt{B}}{\stateTuple{\trainSeq}{\tuple{\substHandlerHead{E}{\tid{id}}{A^\prime}, T, W}}{\switches}{\signals}}
	}
\end{mathpar}
\todo{M-à-J des feux}
...
\begin{mathpar}
	\inferrule*[left=\ruleDef{EndEvent}]{
		...
		\redTuple{\guardR}{\bufferFmt{B}}{\stateTuple{\trainSeq}{\tuple{E, T, W}}{\switches}{\signals}}
		\reduces
		\redTuple{\guardS}{\buftail{B}}{\stateTuple{\trainSeq}{\tuple{\popHandlerHead{E}{\tid{id}}, T, W}}{\switches}{\signals}}
	}
\end{mathpar}
...
To each safe step $X = (\vec{x}_i)_{i \in \trains}$, we can associate a system state $C(X)$ as follows. \todo{Décrire la construction des safe steps}. 

With TLA+, we checked that any safe step is actually reliable.  On our network, one finds 58 368 safe steps, they are all reliable. Checking all the path took XXX seconds on YYY. \todo{XXX= nombre de seconde, YYY = décrire la machine (processeur, RAM, etc)}
