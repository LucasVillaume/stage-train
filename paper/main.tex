...
\begin{itemize}
	\item The routing program specifies how the regulator should react to events (i.e. when a train enters a block). A program is a sequence of rules\todo{Je pense qu'il est plus clair d'appeler ça des règles que des statements}. Each rule begins by a guard \evGuard{tid}{n}, specifying that the content of the rule is triggered when the train $tid$ enters its $n$-th block.
\end{itemize}


\paragraph{The Regulator\todo{Version Guillaume}. }
...
%All statements are guarded by a condition $ev(tid,n)$ stating that train $tid$ reached the $n$-th step of its route. The atoms update the regulator fields. 
%A program is given as a sequence of statements. Those follow the grammar:

\begin{grammar}[Regulator's rules][h][regulateur]
...
The waiting field is a set $W \subseteq \blocks \times \Nat \times \trains$. A triple $(pid, n, tid) \in W$  meaning intention is that block $pid$ must be protected against train $tid$ entrance as long as $T(pid) < n$. 
...
%\todo{Je pense que le régulateur ne devrait pas contenir plus d'éléments. Les autres sont en fait des meta-données qui gèrent "le baton de parole" (les priorités sur les règles). Donc, G et B devraient être en dehors du tuple.}


%First, we begin with the train driver model. The train driver has a list of duties giving him some 
