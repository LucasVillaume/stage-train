...
\newcommand{\incr}[1]{{\mbox{\ensuremath{\mathtt{incr}(#1)}}}\xspace}
...
The general principle of our model is to have $N$ agents corresponding to the $N$ trains and one agent corresponding to the Regulator. But first, we have to describe the infrastructure, that is  the playing field. \todo{Déplacer ?}
...
Thus, each train is a quadruple $t = \trainTuple{id}{pos}{dir}{P}$ with $id$ the train's identifier, $\posFmt{pos}$ its position, $\dirFmt{dir}$ its direction and $P$ the sequence of orders. Trains are collected in a sequence $\trainSeq = (t_1, \ldots, t_k)$ where each $t_i$ being the tuple of train $i$. 
...
\begin{itemize}
	...
	\item The waiting set is a set $W \subseteq \blocks \times \Nat \times \trains$. A triple $\tuple{pid, n, tid} \in W$ means block $pid$ must be protected against train $tid$ entering as long as $T(pid) < n$.
\end{itemize}
...
\begin{grammar}[Regulator's rules, with $tid \in \trains$, $n \in  \Nat$, $pid \in \blocks$, $sid \in \turnouts$, $s\in \internalState$.][h][gr:regulator_program]
