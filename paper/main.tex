...
The regulator is characterized by 5 fields. First, it has a general program, next mentioned as $E$. The program is the plan of the regulator. The regulator will follow its rules. A program is given as a sequence of statements. Those follow the grammar:
\begin{grammar}[Regulator's rules][h][regulateur]
    \firstcasesubtil{stmt}{ev(tid, n) : (atom)*}{}
    \firstcasesubtil{atom}{\incr(pid)}{}
    \otherform{att(pid, v)}{}
    \otherform{\turn(sid, s)}{}
    \otherform{auth}{}
 \end{grammar}
with $tid \in \trains$, $n \in  \Nat$, $pid \in \blocks$, $sid \in \turnouts$, $s\in \internalState$.  All statements are guarded by a condition $ev(tid,n)$ stating that train $tid$ reached the $n$-th step of its route. The atoms update the regulator fields. Before we describe the intention of atoms and the other fields, a notation: given some function $f: X \to Y$, $x \in X$ and $y \in Y$, we denote by $f[x \leftarrow y]$, the function:
...
The waiting field is a function $W: \blocks \times \Nat \to \trains \times Bool$. It states that block must be protected. \todo{donner plus de précision sur les intentions ici.}

The history field $H: \trains \to \blocks$ gives the last position of each train recorded  by the regulator. \todo{A quoi cela sert il en bref.}
...
Description par des règles. La gestion du temps est "discrétisée". On ne peut pas "choisir" quelle est la bonne règle. Gestion par des gardes, et des messages. Il y a des soucis grand pas, petit pas
