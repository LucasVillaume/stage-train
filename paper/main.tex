...
To each safe step $X = (\vec{x}_i)_{i \in \trains}$, we can associate a system state $S(X) = \redTuple{\guardI}{ \emptyList}{ \emptyList}{\stateTuple{\trainSeq{}}{\regulator}{\switches}{\signals}}$ with $\stateTuple{\trainSeq{}}{\regulator}{\switches}{\signals}$ defined as follows: 
\begin{itemize}
...
Now, let us turn to $E$. Let $\sigma$ be the unique  shared network configuration in pairs and let 
...
\paragraph{Experiments}  With TLA+, we checked that any safe step is actually reliable.  On our network, one finds 58 368 safe steps, all being reliable. Checking all the path took XXX seconds on YYY. \todo{XXX= nombre de seconde, YYY = décrire la machine (processeur, RAM, etc)}
